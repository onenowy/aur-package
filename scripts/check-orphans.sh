...
# We find directories containing PKGBUILD, then extract the directory name
...
# We query the x86_64 tag for assets ending in .pkg.tar.zst
...
# DB structure: tar.gz -> directories named pkgname-version-release
if [ -f release_dist/shelter-arch-aur.db.tar.gz ]; then
  tar -tzf release_dist/shelter-arch-aur.db.tar.gz | grep -E '^[^/]+/$' | sed 's|/$||' | while read -r entry; do
    ...
    # This assumes version and release don't contain hyphens, which is standard Arch rule
...
# 5a. From .zst files (Artifacts without a local PKGBUILD)
while read -r asset; do
  [ -z "$asset" ] && continue
  basename=${asset%.pkg.tar.zst}
  # Extract pkgname from filename: pkgname-ver-rel-arch
...
# 5b. From DB (Packages registered in DB but not in local PKGBUILDs)
...
# 6. Output result
# If orphans were found, we signal this (e.g. to trigger a DB rebuild or cleanup)
...
# Cleanup
rm -f local_pkgs.txt remote_assets.txt db_pkgs.txt
